...
| `sprite_sources/` → animated portraits | herb id | No |
| `garden_sources/` → growth art | herb id | No |
...
Neither sprite set is regenerated by `build_deck.py`, and neither needs to be: both are
keyed by herb id and both are drawings of the plant, not of the card.

```bash
python3 scripts/build_sprites.py          # animated portraits
python3 scripts/audit_sprites.py          # face placement, connectivity, stale builds
python3 scripts/build_garden_sprites.py   # Garden growth art
```

Redraw one only if the correction changes what the plant *is* — which, for a card that
printed the wrong plant's back, it does not. See `docs/garden-sprites.md` for the growth
art specifically.
