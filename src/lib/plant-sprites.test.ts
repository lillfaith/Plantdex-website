...
describe('plant sprite manifest', () => {
  it('has a portrait for every card in the deck', () => {
    ...
    const missing = PRINTED_CARDS.filter((herb) => !hasSprite(herb.id)).map((herb) => herb.id);
   ...
   * Field Cards started getting portraits. The check exists to catch a TYPO'D OR PHANTOM id
   * in a generated manifest, and `PRINTED_CARDS` only did that job as a side effect of the
   * two sets being the same set. A Field Card is a real herb with a real page, so keeping the
   * deck here would have made an honest sprite indistinguishable from a mistake.
   *
   * The coverage requirement above is untouched: all 45 PRINTED cards must still have one.
   * This only widens what is ALLOWED to have one.
   */
  it('only names real herbs from the catalogue', () => {
    for (const sprite of allSprites()) {
      expect(
        getCatalogueEntry(sprite.herbId),
        `${sprite.herbId} is not a herb in the catalogue`,
      ).toBeDefined();
    }
  });
