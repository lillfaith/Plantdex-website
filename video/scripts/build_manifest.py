...
def probe_video(path: Path) -> dict:
    """Frame size, fps, frame count and duration of a footage file."""
    env = {**os.environ, "LD_LIBRARY_PATH": str(BIN)}
    out = subprocess.run(
        [str(BIN / "ffprobe"), "-v", "error", "-select_streams", "v:0", "-print_format", "json",
         "-show_entries", "stream=width,height,r_frame_rate,nb_frames:format=duration", str(path)],
        check=True, capture_output=True, env=env, text=True,
    ).stdout
    j = json.loads(out)
    st = j["streams"][0]
    n, d = (int(x) for x in st["r_frame_rate"].split("/"))
    return {"width": st["width"], "height": st["height"], "fps": n / d,
            "frames": int(st["nb_frames"]), "duration": float(j["format"]["duration"])}
...
def main() -> int:
    ...
    supplied = {s["dest"]: s for s in sources if s.get("dest")}
    footage = {
        Path(f).stem: {"file": f, "original": supplied.get(f, {}).get("file"),
                       **{k: files[f][k] for k in ("width", "height", "fps", "frames", "duration")}}
        for f in sorted(files) if f.startswith("07-footage/")
    }
    photos = {
        Path(f).stem: {"file": f, "original": supplied.get(f, {}).get("file")}
        for f in sorted(files) if f.startswith("08-photos/")
    }
...
def markdown(m: dict) -> str:
    L = ["# Plantdex ad asset manifest", "",
         ...
         "## Sources", ""]
    L += [f"- `{s['zip']}` — {s['files']} files, sha256 `{s['sha256'][:16]}…`" for s in m["sources"] if s.get("zip")]
    L += [f"- owner-supplied `{s['file']}` → `{s['dest']}`, original sha256 `{s['sha256'][:16]}…` (re-encoded; location metadata removed)"
          for s in m["sources"] if not s.get("zip")]
    ...
    L += ["", "## Owner-supplied footage", "", "| Key | Size | Length | Original |", "|---|---|---|---|"]
    L += [f"| {k} | {v['width']}×{v['height']} @ {v['fps']:g}fps | {v['duration']:.2f}s ({v['frames']} frames) | `{v['original']}` |"
          for k, v in m["footage"].items()]
    L += ["", "## Owner-supplied photos", "", "| Key | Size | Original |", "|---|---|---|"]
    L += [f"| {k} | {m['files'][v['file']]['width']}×{m['files'][v['file']]['height']} | `{v['original']}` |" for k, v in m["photos"].items()]
    L += ["", "The box back (`box-back`, and the back in `box-in-hand`) prints \"medicinal\" and \"Healing traits\" — ",
